...
class_roll1016 %<>% 
  data.frame
class_roll <- read_excel("./data/class_roll231113.xlsx", 
                        range = "B1:I912")
class_roll %<>% 
  data.frame
str(class_roll)
names(class_roll) <- c("college", "dept", "year", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll1016$group[match(class_roll$id, class_roll1016$id)]
quiz <- 
...
absentee$group %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2023-11-27 09:00:00", 
                                 quiz$time, tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2023-11-27 09:00:00", 
...
교통사고로 다리가 부러진 것은 마찬가지인데 다른사람들의 상태가 어떤가에 따라 드는 생각의 차이를 알아 보았습니다. 이러한 생각조차 상황에 따라 달라지고 있습니다. 
