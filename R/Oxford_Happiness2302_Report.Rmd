...
oxford$days <- as.numeric(difftime("2023-10-30 09:00:00", oxford$Time1, units = 'days'))
oxford$hours <- as.numeric(difftime("2023-10-30 09:00:00", oxford$Time1, units = 'hours'))
```
...
#   pander
oxford[wrong_group, c("id", "Name", "group2", "Group")] %>%
  `[`(order(.$id), ) %>%
  `colnames<-`(c("학번", "이름", "랜덤화출석부", "옥스포드행복설문지")) %>%
