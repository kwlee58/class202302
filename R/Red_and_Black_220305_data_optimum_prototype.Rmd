...
str(class_roll)
class_roll$group <- 
  class_roll0225$group[match(class_roll$id, class_roll0225$id)]
str(class_roll)
```

### 2월 25일자 출석부에만 나오는 사람들의 분포
...
class_roll0225$group %>%
  ...
  table %>%
  pander
N_stay <- class_roll0225$group %>%
  `[`(class_roll0225$id %in% id_stay) %>%
  length
red_stay <- class_roll0225$group %>%
  `[`(class_roll0225$id %in% id_stay) %>%
  table %>%
  `[`(1) %>%
  unname
...
```{r, echo = FALSE}
str(class_roll0225[class_roll0225$id %in% id_out, ])
```
...
tbl1 %>%
  pander
class_roll <- class_roll[, names(class_roll0225)]
X1min <- tbl1 %>%
